...
export const NEO = axios.create({
  // baseURL: `http://vip-registration.tangofestival-bonn.de/service/`,
  baseURL: `http://82.165.102.48:7474/db/data/transaction/commit/`,
  headers: {
  // Authorization: 'Bearer {token}'
    'Content-Type': 'application/json; charset=utf-8',
...
export const actions = {
  ...
  async getNeo ({state}, name) {
    let statements = {
      statements: [
        { statement: 'MATCH (p:Person)-[MEMBEROF]->(g:Group {name: "' + name + '"}) RETURN p' }
      ]
    }
  ...
  async setManagerRelation ({ commit, state }, obj) {
    let statements = {
      statements: [
        { statement: 'MATCH (p:Person {id: "' + obj[0] + '"}), (g:Group {id: "' + obj[1] + '"}) MERGE (p)-[r:MANAGES]->(g) RETURN p,g ' },
        { statement: 'MATCH (p:Person {id: "' + obj[0] + '"}), (g:Group {id: "' + obj[1] + '"}) MERGE (g)-[r:MANAGEDBY]->(p) RETURN p,g ' }
      ]
    }
    // let json = statements.replace(/"([^,]+?)":/g, '$1:')
    console.log('statements', statements)
